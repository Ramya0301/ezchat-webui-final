...
export const convertExternalChatFormat = (wrongJson) => {
	const messages: any[] = [];
	const history: Record<string, any> = {};

	// Helper function to recursively process messages and their children
	const processMessage = (msg) => {
		const newMsg = {
			id: msg.messageId,
			parentId: msg.parentMessageId === "00000000-0000-0000-0000-000000000000" ? null : msg.parentMessageId,
			childrenIds: msg.children.map(child => child.messageId),
		...
		history[msg.messageId] = newMsg;

		// Recursively process children
		if (msg.children && msg.children.length > 0) {
			msg.children.forEach(child => processMessage(child));
		}
	};

	// Process all top-level messages
	wrongJson.messages.forEach(msg => processMessage(msg));

	// Create the correctly formatted chat object
	const convertedChat = {
		id: wrongJson.conversationId,
		user_id: wrongJson.user_id || "", // Populate if available
		title: wrongJson.title,
		chat: {
			id: wrongJson.conversationId,
			title: wrongJson.title,
			models: wrongJson.options.model ? [wrongJson.options.model] : [],
			params: wrongJson.options || {},
			history: {
				messages: history
			},
			messages: messages
		},
		created_at: Math.min(...messages.map(msg => msg.timestamp)),
		updated_at: Math.max(...messages.map(msg => msg.timestamp)),
		tags: [],
		files: []
	};
